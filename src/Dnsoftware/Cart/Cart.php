...
use Dnsoftware\Cart\Helpers\Helpers;
use Dnsoftware\Cart\Validators\CartItemValidator;
use Dnsoftware\Cart\Exceptions\UnknownModelException;

/**
 * Class Cart
 * @package Dnsoftware\Cart
 */
class Cart
{
    ...
    public function addItemCondition($productId, $itemCondition)
    {
        if ($product = $this->get($productId)) {
            $conditionInstance = "\\Dnsoftware\\Cart\\CartCondition";

            if ($itemCondition instanceof $conditionInstance) {
                // we need to copy first to a temporary variable to hold the conditions
                // to avoid hitting this error "Indirect modification of overloaded element of Dnsoftware\Cart\ItemCollection has no effect"
    ...
    public function condition($condition)
    {
        ...
        if (!$condition instanceof CartCondition) throw new InvalidConditionException('Argument 1 must be an instance of \'Dnsoftware\Cart\CartCondition\'');
